...
```{r}
tf_parsed <- tf_parsed_dates %>%  filter(commit_month < 'Jul 2017') # exclude July data
...
tf_commits_by_author <- merge(tf_commits_by_author, tf_authors, by=c("commit_month"))
...
```{r}
tf_commits_freq_by_author <- tf_commits_by_author %>% 
  group_by(author, author_commits_log) %>%
  summarise(commits_log_freq = n(), # frequency of this proportion of commits
            commits_max = max(num_author_commits),
            commits_min = min(num_author_commits),
            commits_pct_max = round(max(commits_pct), 4),
            commits_pct_min = round(min(commits_pct), 4)) %>% 
  group_by(author) %>%
  mutate(commits_log_most_freq = max(commits_log_freq)) %>%
  filter(commits_log_most_freq == commits_log_freq) # only get the most frequent percents

tf_commits_freq_by_author_summary <- tf_commits_freq_by_author %>%
  group_by(author) %>%
  summarise(
    freq_strength=n(), # variability
    commits_log_min = min(author_commits_log),
    commits_log_max = max(author_commits_log),
    commits_pct_min = min(commits_pct_min),
    commits_pct_max = max(commits_pct_max),
    commits_log_freq = first(commits_log_most_freq)
  ) %>% mutate(commits_freq_score = round((commits_log_freq/freq_strength)/commits_log_freq, 1))

tf_commits_by_author <- merge(tf_commits_by_author, 
                              tf_commits_freq_by_author_summary %>% 
                                select(author, commits_pct_max, commits_log_max, 
                                       commits_freq_score),
                              by="author")

rm(tf_commits_freq_by_author, tf_commits_freq_by_author_summary)
```
...
```{r}
tf_authors_total <- tf_parsed_dates %>% group_by(author) %>% summarise(num_total_commits=n())
...
## Most Frequent Commit Pattern

This looks at the same plot above but adds the dimension of what's "normal" for the authors in the plot. Do they appear to be consistent with their most common values or do they tend to vary a lot?

Generally authors are consistent. We don't see a lot of cases where authors who typically have a low number of commits having random spikes of high numbers of commits.

One caveat to keep in mind is some commits being evaluated as belonging to seperate authors may belong to the same author. Github reports 73 less authors than were determined using the author's name from the commit. At a minimum there seems to at least be consistency in commit activity per author name.

```{r}
ggplot(data = tf_commits_by_author, 
       aes(x = num_authors, y=num_author_commits, 
           colour=factor(commits_log_max))) +
  geom_jitter() +
  xlab("Authors") +
  ylab("Commits per Author") +
  guides(colour=guide_legend(title="Most Freq Log"))
