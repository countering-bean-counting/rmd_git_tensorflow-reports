...
```{r}
# Extract github from noreply emails

tf_gh_authors <- tf_parsed_authors %>%
  group_by(email) %>%
  summarise(sha_id=first(sha_id)) %>%
  separate(email, c("email_user", "email_host"), "@") %>%
  mutate(
    github_login=ifelse(email_host == "users.noreply.github.com", email_user, NA)
  )

tf_gh_authors <- merge(tf_parsed_authors, tf_gh_authors, by="sha_id", all=TRUE)
```
