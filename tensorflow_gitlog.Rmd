...
tf_outfile <- ''
system('cd tensorflow; git log --no-merges --date=short --pretty=format:"%ad %an" > ../data/tensorflow_gitlog.txt')
...
```{r}
tf_authors_commits_long <- melt(tf_authors_commits %>% select(commit_month, num_authors, num_commits), id="commit_month")
