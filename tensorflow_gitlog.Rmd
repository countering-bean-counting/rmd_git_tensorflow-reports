...
```{r}
# manual updates for known authors
...
tf_gh_authors_by_email <- tf_parsed_emails %>%
  group_by(email, name) %>%
  summarise(num_commits = n()) %>% select(email, name)

tf_gh_authors_by_email2 <- tf_parsed_emails %>%
  group_by(name) %>%
  summarise(email_id=first(email), sha_id=first(sha)) %>%
  select(name, email_id, sha_id)

tf_gh_authors_by_email <- merge(tf_gh_authors_by_email, tf_gh_authors_by_email2, by="name")

tf_gh_authors_by_name <- tf_gh_authors_by_email %>%
  group_by(email) %>%
  mutate(author=first(name))

# make a lookup of authoritative names and emails
tf_gh_authors_lookup <- merge(tf_gh_authors_by_name %>% select(name, email, author),
                              tf_gh_authors_by_email %>% select(email, name, email_id, sha_id),
                              by=c("name", "email"), all=TRUE)

tf_parsed_authors <- merge(tf_parsed_emails, tf_gh_authors_lookup, by=c("email", "name"), all=TRUE)

rm(tf_gh_authors_by_email, tf_gh_authors_by_email2, tf_gh_authors_by_name, tf_parsed_emails)
...
```{r}
tf_parsed <- tf_parsed_authors %>% filter(commit_month < "July 2017") # exclude July
...
The number of unique authors has grown steadily per month. March 2017 shows a spike, so it could be interesting to see what may have happened to cause it.
...
How many Google authors do we see?

```{r}
# filter out known email provider domains
tf_authors_hosts <- tf_parsed %>%
  filter(!email_host %in% c("gmail.com", 
                            ...
                            "163.com",
                            "outlook.com")) %>%
  group_by(author, email_host) %>%
  summarise(num_commits = n()) %>%
  group_by(email_host) %>%
  summarise(total_authors=n(), total_commits=sum(num_commits))

tf_authors_hosts_month <- tf_parsed %>%
  group_by(commit_month, author, email_host) %>%
  summarise(num_commits = n()) %>%
  group_by(commit_month, email_host) %>%
  summarise(num_authors=n(), num_commits=sum(num_commits))

# only include hosts with overall contributions > 1
tf_authors_hosts_month_merge <- merge(tf_authors_hosts_month, 
                                      tf_authors_hosts %>% select(email_host, total_authors, total_commits),
                                      by="email_host")

# All emails vs google.com
ggplot(tf_authors_hosts_month %>% 
         mutate(is_google=ifelse(email_host=="google.com", "Google", "Not Google")),
...
# Non-email-provider addresses and more than one overall author (otherwise there are too many to plot)
ggplot(tf_authors_hosts_month_merge %>% filter(total_authors > 1 & email_host != "google.com"),
       aes(x=factor(commit_month), y=num_authors, fill=reorder(email_host, -total_authors))) + 
  geom_bar(stat="identity") +
  xlab("Months") +
  ylab("Authors") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
Are they responsible for most of the 1-month contributors?
...
ggplot(tf_commits_hosts %>% filter(host_authors > 1),
       aes(x=factor(commit_month), y=num_hosts, fill=reorder(email_host, -total_authors))) + 
  geom_bar(stat="identity") +
  xlab("Months") +
...
# Github Pulse Report Comparison

How different are metrics in the Gitlog compared to what Github reports?

This report shows the same number of commits and 1 additional author. Previous indications show the correct number of authors (995) total. This discrepancy is due to a timezone mismatch, illustrated below.
...
total_commits_pulse_check <- sum(tf_pulse_check_authors$num_author_commits)
...
* commit months per author (number of months with at least one commit)
* change in proportion of min/max commits per month
* author commit proportion per month change

## Proposed Theories

### If a company has influence in a project, the majority of authors associated with that company will identify the association in their commit activity.

It's not clear how accurate the company affiliation in the Github profile is. 

* Name Normalization: This is a free-form text field and the names need to be normalized. 
* No History: There is no public history of Github profile changes. If a contributor changes companies, all of their activity will be associated with the new company.
* Authors could be contributing to the project DESPITE their company affiliation, not because of it.

If an author's contributions are condoned by their company, they should be using company resources to make those contributions. Committers who use their work email address likely have a strong company affiliation or else they wouldn't be using company resources in such a publicly visible space. Committers who do not use their work email likely want to obscure their employer affiliation.
...
* A prolific committer can easily skew total commits
* Committers identified through Github profiles have no historical context (see above)

#### Commit months per company

On the other hand, measuring whether a company had at least one author committing to the repository using their company email address will indicate whether or not the company had any involvement for a given time period. This also provides a clear historical pattern of activity, in particular how long the company has been represented in the project.
...
How the proportion of commits changes over time (what proporton is a min/max)
