...
ggplot(tf_authors_hosts %>% filter(total_authors > 1),
       aes(x=email_host, y=total_authors)) + 
...
ggplot(tf_authors_hosts_month_merge %>% filter(total_authors > 1),
       aes(x=factor(commit_month))) + 
  geom_line(aes(y=num_authors, colour=email_host, group=email_host)) +
  xlab("Months") +
  ylab("Authors") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
Are the responsible for most of the 1-month contributors?
...
The key disadvantage is the inability to easily consolidate authors. While this may potentially misrepresent the distribution of commits for a small author base, a project with a large number of authors should not see a big problem. Number of unique authors can be checked against Github for an approximate margin of error.
...
Additionally, non-human users in projects with a high proportion of drive-by authors can be identified by a significantly higher proportion of commits than the other authors. TODO: how does a non-human author compare to a long-term author?
